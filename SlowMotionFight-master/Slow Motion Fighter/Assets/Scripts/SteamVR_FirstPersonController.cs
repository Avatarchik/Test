...
public class SteamVR_FirstPersonController : MonoBehaviour
{
    ...
    private MotionPathCreator currMPCreator = null;
    ...
    void UpdateGrabbableObjects()
    {
        if (canGrabObject != null)
        {
            ...
            {
                currMPCreator = canGrabObject.GetComponent<MotionPathCreator>();
                currMPCreator.startDraw();
            }
            else if (device.GetTouch(SteamVR_Controller.ButtonMask.Trigger) && currMPCreator != null)
            {
                currMPCreator.updateDraw(controllerAttachPoint.transform.position);
            }
            else if (device.GetTouchUp(SteamVR_Controller.ButtonMask.Trigger) && currMPCreator != null)
            {
                currMPCreator.endDraw();
                ToggleGrabbableObjectHighlight(false);
                canGrabObject = null;
                currMPCreator = null;
            }
    ...
    {
        if (currMPCreator == null && collider.tag == "touchPoint")
        {
            ToggleGrabbableObjectHighlight(false);
            canGrabObject = null;
        }
